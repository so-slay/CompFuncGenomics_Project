...
# For ROC etc (stats and visualization)
...
runTime_df = runtime_df = pd.DataFrame(runtimes, columns=["order", "helper_runtime"])

runtime_file = output_dir / "runtimes.out"
runtime_df.to_csv(runtime_file, sep="\t", index=False)

helperRunTime = mm.helperRunTime
runtime_df["helper_constant_col"] = np.ones(len(runtime_df)) * helperRunTime

# Plot runtimes for all orders
