...
def main():
    ...
    for name,use_atac,use_meth in configs:
        ...
        for c in range(1,23):
            ...
            for ep in range(EPOCHS):
                ...
                roc, pr, preds = evaluate(model, Xval, yval)
                ...
                # snapshot best epoch within this chromosome
                if roc > best_epoch_score:
                    best_epoch_score = roc
                    best_epoch_state = copy.deepcopy(model.state_dict())
                    preds_best = preds  # keep matching preds for plotting

            # restore best epoch weights before plotting and global comparison
            model.load_state_dict(best_epoch_state)

            # now use preds_best for curves (from the actual best epoch)
            fpr, tpr, _ = roc_curve(yval[:,0], preds_best[:,0])
            plt.plot(fpr, tpr)
            plt.savefig(f"roc_chr{c}.png")
            plt.close()

            p, r, _ = precision_recall_curve(yval[:,0], preds_best[:,0])
            plt.plot(r, p)
            plt.savefig(f"pr_chr{c}.png")
            plt.close()

            # global best tracking — now uses best epoch, not final epoch
            if best_epoch_score > best_score:
                best_score = best_epoch_score
                best_state_dict = copy.deepcopy(best_epoch_state)
                best_cfg = (use_atac, use_meth)
                best_in_ch = X.shape[2]  # save architecture info too
    ...
    # replace best_model usage with reconstructed model
